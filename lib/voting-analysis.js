...
class VotingAnalysis {
  constructor(config) {
    ...
    this.votingInfoFetcher = new VotingInfoFetcher(this.config.votingResultsFilePath);
  }
  ...
  getElegibleVotersInformation() {
    ...
    this.info.totalElegible = this.reps.length - this.config.static.newRepsJoinedAfterVoting + this.config.static.repsMovedToAlumniAfterVoting;
